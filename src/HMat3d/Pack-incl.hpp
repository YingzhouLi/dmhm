...
template<typename Scalar>
std::size_t
HMat3d<Scalar>::PackedSize() const
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::PackedSize");
#endif
    std::size_t packedSize = 17*sizeof(int) + 2*sizeof(bool);
...
template<typename Scalar>
std::size_t
HMat3d<Scalar>::Pack( byte* packedHMat ) const
{
#ifndef RELEASE
    ...
    Write( head, zSizeSource_ );
    Write( head, zSizeTarget_ );
    Write( head, xSource_ );
    Write( head, xTarget_ );
    Write( head, ySource_ );
    Write( head, yTarget_ );
...
template<typename Scalar>
std::size_t
HMat3d<Scalar>::Unpack( const byte* packedHMat )
{
#ifndef RELEASE
...
template<typename Scalar>
void
HMat3d<Scalar>::PackedSizeRecursion( std::size_t& packedSize ) const
{
...
template<typename Scalar>
void
HMat3d<Scalar>::PackRecursion( byte*& head ) const
{
...
template<typename Scalar>
void
HMat3d<Scalar>::UnpackRecursion( const byte*& head )
{
    block_.Clear();

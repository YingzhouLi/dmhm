...
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::Multiply (D := H D + D)");
#endif
    ...
    case NODE:
    {
        // Loop over all 64 children, summing in each row
...
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::Multiply (D := H D)");
#endif
    C.SetType( GENERAL );
...
template<typename Scalar>
void
HMat3d<Scalar>::TransposeMultiply
( Scalar alpha, const Dense<Scalar>& B,
  Scalar beta,        Dense<Scalar>& C ) const
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::TransposeMultiply (D := H^T D + D)");
#endif
    hmat_tools::Scale( beta, C );
    switch( block_.type )
    {
    case NODE:
    {
        ...
        Node& node = *block_.data.N;
        for( int t=0,tOffset=0; t<8; tOffset+=node.sourceSizes[t],++t )
        {
            Dense<Scalar> CSub;
            CSub.View( C, tOffset, 0, node.sourceSizes[t], C.Width() );

            for( int s=0,sOffset=0; s<8; sOffset+=node.targetSizes[s],++s )
            {
                Dense<Scalar> BSub;
                BSub.LockedView
...
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::TransposeMultiply (D := H^T D)");
#endif
    C.SetType( GENERAL );
...
template<typename Scalar>
void
HMat3d<Scalar>::AdjointMultiply
( Scalar alpha, const Dense<Scalar>& B,
  Scalar beta,        Dense<Scalar>& C ) const
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::AdjointMultiply (D := H^H D + D)");
#endif
    hmat_tools::Scale( beta, C );
    switch( block_.type )
    {
...
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::AdjointMultiply (D := H^H D + D, non-const)");
#endif
    hmat_tools::Scale( beta, C );
    switch( block_.type )
    {
...
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::AdjointMultiply (D := H^H D)");
#endif
    C.SetType( GENERAL );
    C.Resize( Width(), B.Width() );
...
{
#ifndef RELEASE
    CallStackEntry entry("HMat3d::AdjointMultiply (D := H^H D, non-const)");
#endif
    C.SetType( GENERAL );
    C.Resize( Width(), B.Width() );
    AdjointMultiply( alpha, B, 0, C );
